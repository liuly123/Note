...
#define _CRT_SECURE_NO_DEPRECATE	//禁用安全警告
#define WIN32						//windlows程序
...
int main(int argc, char* argv[])
{
	...
	//关闭内部应用程序
	std::string LogFile = "";
	std::string MulticastAddress = "244.0.0.0:44801";	//多播地址
	bool ConnectToMultiCast = false;					//是否连接多播
	...
	std::string AxisMapping = "ZUp";//Z轴朝上
	...
	for (int i = 0; i != 3; ++i) // 可以断线重连3次
	{
		std::cout << "Connecting to " << HostName << " ..." << std::flush;

		while (!MyClient.IsConnected().Connected)// 连接到server（没连上就不断重试）
		{
			// 直连
			bool ok = false;
		...
		size_t FrameRateWindow = 1000; // 每1000帧计算一次帧率
		size_t Counter = 0;
		clock_t LastTime = clock();

		// while循环读取数据（直到按下按键停止）
		//#ifdef WIN32
				while (!Hit())
		//#else
		//		while (true)
		//#endif
		{
			...
			{
				// Sleep一会儿，这样我们就不会在频繁的轮询中浪费CPU的时间
				#ifdef WIN32
								Sleep(200);
				#else
								sleep(1);
